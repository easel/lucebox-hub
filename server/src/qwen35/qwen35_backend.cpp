...
bool Qwen35Backend::run_ar_decode_path(int committed, int n_gen,
    ...
    return do_ar_decode(committed, n_gen, out_tokens, io,
                        budget_hook, forced_close_out, degenerate_close_out,
...
bool Qwen35Backend::do_spec_decode(int committed, int n_gen,
    ...
    if (!can_spec) {
        ...
        // one token at a time. Route through the virtual AR path so MoE/hybrid
        // backends keep their specialized decode while the budget hook still
        // fires when spec-decode is unavailable. Soft-close pointer also forwards
        // so close_kind="soft" can be attributed correctly on the fallback path.

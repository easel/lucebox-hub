...
bool Qwen35LayerSplitAdapter::init() {
    ...
    disk_snapshot_contexts_.resize(PREFIX_SLOTS, nullptr);
    disk_snapshot_buffers_.resize(PREFIX_SLOTS, nullptr);
    disk_snapshot_backends_.resize(PREFIX_SLOTS, nullptr);
...
bool Qwen35LayerSplitAdapter::snapshot_save(int slot) {
    ...
    return rebuild_disk_snapshot(slot);
}
...
void Qwen35LayerSplitAdapter::snapshot_free(int slot) {
    ...
    if (slot >= 0 && disk_snapshot_contexts_.size() == (size_t)PREFIX_SLOTS &&
...
bool Qwen35LayerSplitAdapter::snapshot_used(int slot) const {
    ...
    const bool has_disk_context =
        disk_snapshot_contexts_.size() == (size_t)PREFIX_SLOTS &&
        slot >= 0 && slot < (int)disk_snapshot_contexts_.size() &&
        disk_snapshot_contexts_[(size_t)slot];
    for (const auto & snap : snaps) {
        if (!snap.ctx && !has_disk_context) return false;
    }
    if (snapshot_prefill_logits_.size() != (size_t)PREFIX_SLOTS ||
...
bool Qwen35LayerSplitAdapter::rebuild_disk_snapshot(int slot) {
    ...
    struct CopyPair { ggml_tensor * src = nullptr; ggml_tensor * dst = nullptr; };
...
bool Qwen35LayerSplitAdapter::snapshot_adopt(int slot,
    ...
    for (auto & snap : snaps) {
        // The adopted disk snapshot owns one shared ggml context/buffer for all
        // shard tensor references. Keep ownership in disk_snapshot_* only so
        // free_prefix_snapshot() does not double-free the shared objects.
        snap.ctx = nullptr;
        snap.buf = nullptr;
    }
    disk_snapshot_contexts_[(size_t)slot] = ctx;

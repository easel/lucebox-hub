...
int run_dflash_draft_ipc_daemon(const char * draft_path,
    ...
    auto drain_payload = [&](size_t bytes) -> bool {
        if (payload_fd < 0 || bytes == 0) return true;
        std::vector<char> scratch(std::min<size_t>(bytes, 64 * 1024));
        size_t remaining = bytes;
        while (remaining > 0) {
            const size_t chunk = std::min(remaining, scratch.size());
            if (!read_exact_fd(payload_fd, scratch.data(), chunk)) return false;
            remaining -= chunk;
        }
        return true;
    };
        ...
        if (cmd == "feature_slice_pipe") {
            ...
            if (payload_fd < 0 || capture_idx < 0 || capture_idx >= n_tgt_layers ||
                ...
                std::fprintf(stderr, "[draft-ipc-daemon] bad feature_slice_pipe: %s\n",
                             line.c_str());
                if (payload_fd >= 0 && bytes > 0 && !drain_payload(bytes)) {
                    std::fprintf(stderr, "[draft-ipc-daemon] drain feature_slice pipe failed\n");
                    stream_status(stream_fd, -1);
                    break;
                }
                stream_status(stream_fd, -1);
        ...
        if (cmd == "propose_pipe") {
            ...
            if (payload_fd < 0 || committed < 0 || ctx_len <= 0 ||
                ...
                std::fprintf(stderr, "[draft-ipc-daemon] bad propose_pipe: %s\n",
                             line.c_str());
                if (payload_fd >= 0 && bytes > 0 && !drain_payload(bytes)) {
                    std::fprintf(stderr, "[draft-ipc-daemon] drain noise pipe failed\n");
                    stream_status(stream_fd, -1);
                    break;
                }
                stream_status(stream_fd, -1);

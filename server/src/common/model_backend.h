...
struct ModelBackend {
    ...
    GenerateResult generate(const GenerateRequest & req, const DaemonIO & io) {
        ...
        std::fprintf(stderr,
            "[backend] spec-decode produced no visible output after %.3f s decode; "
    ...
    GenerateResult restore_and_generate(int slot, const GenerateRequest & req,
        ...
        std::fprintf(stderr,
            "[backend] restored spec-decode slot=%d produced no visible output "
            "after %.3f s decode; retrying with AR decode\n",

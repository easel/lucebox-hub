...
Compares cold (--cache-prefix-ram=0) vs warm (--cache-prefix-ram=256MiB).
...
def run_config(label: str, port: int, prefix_ram_mib: int, user_turns: list[str],
                max_tokens: int, log_path: Path) -> list[float]:
    """Spin up server with the requested prefix RAM cache pool."""
    log_f = open(log_path, "w")
    proc = subprocess.Popen(
        [str(SERVER_BIN), str(TARGET),
         "--draft", str(DRAFT),
         "--max-ctx", "4096", "--port", str(port),
         "--cache-prefix-ram", f"{prefix_ram_mib}MiB"],
    ...
    print(f"\n--- {label} (cache-prefix-ram={prefix_ram_mib}MiB) ---", flush=True)
...
def main():
    ...
    # Cold config: cache disabled, so every turn re-prefills full history.
    cold = run_config("COLD (cache disabled)", port=18290, prefix_ram_mib=0,
                       user_turns=user_turns, max_tokens=args.max_tokens,
                       log_path=Path("/tmp/bench_cold.log"))

    # Warm config: cache enabled, so turn-boundary snapshots can hit.
    warm = run_config("WARM (cache enabled)", port=18291, prefix_ram_mib=256,
                       user_turns=user_turns, max_tokens=args.max_tokens,

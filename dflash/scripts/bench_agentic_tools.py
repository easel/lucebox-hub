...
def run_case_once(
    ...
    expected_schema = (_tool_for(case["expected"])["function"]["parameters"])
    arg_valid = False
    parsed_args: list[dict[str, Any]] = []
    for call in calls:
        fn = call.get("function") or {}
        if fn.get("name") != case["expected"]:
            continue
        ...
        if isinstance(args, dict):
            ...
            if all(isinstance(args.get(name), str) and args.get(name) for name in required):
                arg_valid = True
    return {
        "name": case["name"],
        "expected": case["expected"],
        "ok": case["expected"] in names and arg_valid,
        "http_status": http_status,
        "finish_reason": choices[0].get("finish_reason") if choices else None,
        "tool_names": names,
        "tool_args_valid": arg_valid,

...
Last refresh: `2026-05-31T15:55:00-04:00`
...
| #321 | `feat-mixed-backend-layer-split-runtime` | `87fe7655` | partial selective salvage / related to #325 | #321 has unique IPC/state hardening commit `87fe7655`; #325 is feature-superset-like but not an exact head superset. This run ported the conflict-free placement-config foundation: `DevicePlacement::layer_split_backends`, mixed-backend list parsing, `DevicePlacement::is_mixed_layer_split`, `layer_split_backend()`, backend+GPU duplicate validation, and unit coverage. Remaining unique behavior includes `RemoteTargetShardConfig`, `--target-shard-ipc-*`, Qwen35 target-shard IPC daemon/client plumbing, state reset/snapshot/save/restore hardening, and the latest target-shard shared-payload / prefix-snapshot cleanup deltas. The next useful action remains a staged selective port of the target-shard IPC control plane before reattempting remaining #325 mixed-backend/Laguna disk-cache pieces. |
...
- `/tmp/luce-auto-cron-20260531-153628/reconcile` (current source/manifest refresh worktree; #321 placement-config foundation slice promoted here)

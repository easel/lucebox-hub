...
Last refresh: 2026-05-29T09:14:06-04:00
Current base: `origin/main` `8782d07a`
Current integration tip before this refresh: `easel/auto-integration` `f2f9cd90`
...
already based on current `origin/main` (`0` behind / `433` ahead in the
...
`/tmp/luce-auto-cron-20260529-090855`; they still require selective current-layout
ports or are superseded.
...
- `date -Is` -> `2026-05-29T09:08:04-04:00` for preflight; manifest timestamp
  refreshed at `2026-05-29T09:14:06-04:00`.
...
- Reconciliation worktree `/tmp/luce-auto-cron-20260529-090855/reconcile` was
...
- `git rev-list --left-right --count origin/main...HEAD` in the reconciliation
  worktree reported `0` behind and `433` ahead before the manifest-only refresh.
...
- New tmux-driven Codex delegation was attempted for #94 and produced a usable
  read-only feasibility report at `/tmp/luce94-codex-20260529-090910-report.txt`,
  concluding that #94's Qwen3.6 draft SWA behavior and bare-filename
  `config.json` lookup are already superseded by the current auto-integration
  tree and that any resolution should keep the current side.
...
|---:|---|---:|---|---|
| #237 | `feat/dflash-mtp-foundation` | `02c6a6c4` | blocked-needs-human / salvage-port | Fresh probe worktree `/tmp/luce-auto-cron-20260529-090855/pr-237-probe` conflicts in old-layout `dflash/` server/backend files plus current `server/CMakeLists.txt`, common MTP files, Qwen35 loader/graph/backend/dflash target files, and tests. Prior Codex read-only delegation found this feasible only as a selective manual port: MTP core files, `ModelBackend` MTP support, native/external/auto source selection, target hidden captures, and GGUF `nextn_predict_layers` handling must be ported into the current layout while preserving current PFlash/C2, remote draft, qwen35moe, layer-split, and thinking-budget paths. |
| #221 | `feat/mtp-prefix-warm-ghost` | `05502974` | blocked-needs-human / dependent salvage-port | Fresh probe worktree `/tmp/luce-auto-cron-20260529-090855/pr-221-probe` conflicts on old `dflash/` scripts/backend files, current common/Qwen35 MTP, metadata, prefix snapshot, graph, and tests, and brings old benchmark/results assets. New Codex delegation says to mine prefix-cache WARM behavior (`snapshot_head_kv`, `restore_head_kv`, `warm_head_kv_range`, `head_kv_warm_`), partial/range warm recovery, the NextN raw-`block_count` load-filter fix, dual-speculator routing ideas, and focused `test_prefix_cache_mtp.cpp` cases only after a PR-237-equivalent current-layout MTP foundation lands. Drop old qwen36 names, old Python server behavior, old inline snapshot storage, archived result payloads, and behavior already superseded by current `skip_park` / `cache_.last_tok` handling. |
| #154 | `xabicasa/dflash-mtp-speculative-loop` | `2f4ede79` | blocked-needs-human / dependency | Fresh probe worktree `/tmp/luce-auto-cron-20260529-090855/pr-154-probe` conflicts on old `dflash/CMakeLists.txt`, moved MTP docs/tests and `f16_convert.cu`, plus core Qwen35/internal target files. Mine linear MTP decode semantics after current-layout Qwen35 MTP exists. |
| #153 | `xabicasa/dflash-mtp-integrated` | `e9b17cb1` | blocked-needs-human / dependency | Fresh probe worktree `/tmp/luce-auto-cron-20260529-090855/pr-153-probe` conflicts on old CMake, moved MTP docs/tests and `f16_convert.cu`, and the same core Qwen35/internal target files. Mine loader/graph/cache/test ideas after current-layout Qwen35 MTP exists. |
| #135 | `xabicasa/dflash-multi-request-scheduler-batched-target-step` | `561b0ac1` | blocked-needs-human / selective-port | Fresh probe worktree `/tmp/luce-auto-cron-20260529-090855/pr-135-probe` conflicts in `server/src/internal.h`, `server/src/qwen35/qwen35_target_graph.cpp`, and `server/test/test_dflash.cpp`. Recent Codex delegation says the feature is practical but medium-sized: add qwen35 `n_seqs`/batched one-token target-step support in current graph builders and `Qwen35DFlashTarget`, extract scheduler bucket selection with tests, then wire backend decode-session slots and opt-in scheduler APIs. Keep sampling, DDTree tree mode, layer split, and MoE disabled until explicitly validated. |
| #137 | `xabicasa/dflash-build-cmake-sm89-bsa` | `297fc74e` | suggested-close/superseded | Fresh probe worktree `/tmp/luce-auto-cron-20260529-090855/pr-137-probe` only conflicts on deleted old `dflash/CMakeLists.txt`. Ask author to close or retarget to current `server/CMakeLists.txt` if anything remains. |
| #94 | `feat/dflash-qwen36-swa-draft` | `d2f9c9dd` | suggested-close/superseded | Fresh probe worktree `/tmp/luce-auto-cron-20260529-090855/pr-94-probe` conflicts in `server/src/draft/draft_graph.cpp`, `draft_safetensors_loader.cpp`, and `server/src/internal.h`; tmux-driven Codex report `/tmp/luce94-codex-20260529-090910-report.txt` concluded the useful behavior is already present in current auto-integration: Qwen3.6 DFlash SWA metadata loading, bare-filename `config.json` lookup, and a stronger shared-runtime `causal_mask_swa` path. Ask author/maintainers to close unless they can identify an unported test case. |
| #48 | `fix/consumer-blackwell-auto-detect` | `858b84b6` | suggested-close/superseded | Fresh probe worktree `/tmp/luce-auto-cron-20260529-090855/pr-48-probe` only conflicts on deleted old `dflash/CMakeLists.txt`. Close or retarget to current `server/CMakeLists.txt` if still needed. |
...
- `/tmp/luce-auto-cron-20260529-090855/reconcile`
- `/tmp/luce-auto-cron-20260529-090855/pr-237-probe`
- `/tmp/luce-auto-cron-20260529-090855/pr-221-probe`
- `/tmp/luce-auto-cron-20260529-090855/pr-154-probe`
- `/tmp/luce-auto-cron-20260529-090855/pr-153-probe`
- `/tmp/luce-auto-cron-20260529-090855/pr-137-probe`
- `/tmp/luce-auto-cron-20260529-090855/pr-135-probe`
- `/tmp/luce-auto-cron-20260529-090855/pr-94-probe`
- `/tmp/luce-auto-cron-20260529-090855/pr-48-probe`
- `/tmp/luce-auto-cron-20260529-090855`

Fresh direct-merge probe logs were written under `/tmp/luce-auto-cron-20260529-090855/pr-*-merge.log`.
...
range-warm, NextN load-filter, and dispatcher/routing behavior plus #153/#154's
native/integrated MTP semantics. #135 is a separate current-layout selective port
that should be split into qwen35 batched one-token target-step support, scheduler
bucket selection tests, backend decode slots, and opt-in scheduler APIs. #137 and

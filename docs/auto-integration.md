...
Last refresh: `2026-05-31T12:25:25-04:00`
Current base: `origin/main` `8305b6c2`
Previous integration tip: `easel/auto-integration` `7edfbdbc`
Current integration source tip before this refresh: `7edfbdbc`
...
The next useful integration work remains a dedicated selective port, not another direct merge. Highest-value candidates are now: (1) validate the newly promoted #135 daemon cache-slot and request-state scaffolds (`LIST_TARGET_CACHE_SLOTS`, `LIST_REQUESTS`, and `CANCEL`) under a populated CUDA/server build; (2) continue #237 using the retained `/tmp/luce-port-pr237-qwen-mtp-20260531-083154` draft only after populating `server/deps/llama.cpp` or otherwise obtaining CI-backed compile evidence for the Qwen35 MTP module/graph/loader slice; (3) #135 runtime validation for the already-ported batched target-graph, request-tagged stream framing, and target-feature capture slices, then non-batched `SCHED_STEP`/`SCHED_DRAIN` introspection and diagnostic-only batch probing before live copyback/target-step mutation; (4) validate #305's newly ported Qwen35MoE gallocr-reuse/full-chunk FFN path under a populated CUDA server build before mining any remaining GPU-resident `act_cur` / async logits projection changes; and (5) #305's larger Laguna hot/cold MoE hybrid path if Laguna XS.2 residency is still desired. #153/#154 should be mined after #237 has current-layout Qwen35 runtime wiring for linear MTP decode semantics, and #221 only for any still-missing prefix-warm ideas. #137, #48, and #94 are represented by superseding current-layout code and can be closed or retargeted only if authors can identify a minimal missing delta.

...
|---:|---|---:|---|---|
| #325 | `feat-layer-split-disk-prefix-cache` | `b47fb3aa` | pending current-layout port / no longer strict superset of #321 | Direct merge conflicts across the current stack's backend IPC, DFlash draft IPC, layer-split runtime, Gemma4/Laguna/Qwen35 layer-split adapters, backend IPC main, Laguna target loader, Qwen35 layer-split code, and disk-prefix-cache snapshot hooks. Codex found the smallest safe first slice is same-backend Qwen35 layer-split disk-prefix-cache export/adopt (`snapshot_ref` / `snapshot_adopt` through `LayerSplitBackend` and `Qwen35LayerSplitAdapter`), while deferring Laguna and mixed-backend IPC. |
| #321 | `feat-mixed-backend-layer-split-runtime` | `87fe7655` | pending current-layout port / related to #325 | #321 now has unique IPC/state hardening commit `87fe7655`; #325 is feature-superset-like but not an exact head superset. Unique missing behavior includes `RemoteTargetShardConfig`, `--target-shard-ipc-*`, `DevicePlacement::is_mixed_layer_split`, Qwen35 target-shard IPC daemon/client plumbing, and the latest hardening delta. |
...
The next useful integration work remains a dedicated selective port, not another direct merge. Highest-value candidates are now: (1) port the #325 same-backend Qwen35 layer-split disk-prefix-cache `snapshot_ref` / `snapshot_adopt` slice identified by Codex, then separately port #321's latest IPC/state hardening and mixed-backend target-shard runtime if still desired; (2) validate the newly promoted #135 daemon cache-slot, request-state, and scheduler-diagnostic scaffolds (`LIST_TARGET_CACHE_SLOTS`, `LIST_REQUESTS`, `CANCEL`, `SCHED_STEP`, and `SCHED_DRAIN`) under a populated CUDA/server build; (3) continue #237 using the retained `/tmp/luce-port-pr237-qwen-mtp-20260531-083154` draft only after populating `server/deps/llama.cpp` or otherwise obtaining CI-backed compile evidence for the Qwen35 MTP module/graph/loader slice; (4) #135 runtime validation for the already-ported batched target-graph, request-tagged stream framing, target-feature capture, slot/request control-plane, and scheduler diagnostics, then diagnostic-only batch probing before live copyback/target-step mutation; (5) validate #305's newly ported Qwen35MoE gallocr-reuse/full-chunk FFN path under a populated CUDA server build before mining any remaining GPU-resident `act_cur` / async logits projection changes; and (6) #305's larger Laguna hot/cold MoE hybrid path if Laguna XS.2 residency is still desired. #153/#154 should be mined after #237 has current-layout Qwen35 runtime wiring for linear MTP decode semantics, and #221 only for any still-missing prefix-warm ideas. #137, #48, and #94 are represented by superseding current-layout code and can be closed or retargeted only if authors can identify a minimal missing delta.

...
Last refresh: `2026-05-31T11:56:44-04:00`
Current base: `origin/main` `8305b6c2`
Previous integration tip: `easel/auto-integration` `e4975c52`
Current integration source tip before this refresh: `e4975c52`
...
The current stack contains 22 exact current open non-draft PR heads and carries selective salvage from three remaining non-ancestor PRs: #305's `DFLASH_EXPERT_BUDGET_PCT` cap plus the current-layout Qwen35MoE batched-FFN gallocr reuse / caller-side `kFfnSafeBatch` removal, #237's common MTP interface/chain-runner/orchestrator foundation, and #135's capture-free `n_seqs` target-graph/cache plumbing, request-tagged daemon stream framing, batched target-feature capture buffer plumbing, and daemon multi-target-cache-slot scaffolding for current qwen35 work. There are currently 28 open non-draft PRs and 8 draft/excluded PRs. This refresh integrated the newly advanced #285 Docker/CLI/bench head (`d3399169`) with a current-stack conflict resolution in `server/src/server/http_server.cpp`, preserving both status-dashboard token broadcasts and #285's Gemma4 `<|channel>*` thinking-token handling. A post-push check found #319 and #285 had both advanced again; this refresh merged #319 `de1c77fe` and #285 `329f6111`, resolving tool-parser and HTTP thinking-token conflicts by preserving both newly added behaviors. A final re-enumeration found new non-draft upstream PR #324, which is the direct Luce-Org/main version of the same visible-empty-DFlash-retry work already carried through #319; it was merged with an ancestry-only/no-tree-change conflict resolution that preserved the status dashboard and broader Gemma4 raw-token handling. A later final re-check found #285 had advanced once more to `d3399169`; that head merged cleanly and added the Bragi pFlash A/B experiment report plus lucebox config path handling tweaks. It also promoted a narrow #135 `server/test/test_dflash.cpp` slice from a tmux-driven Codex attempt: `--target-cache-slots` / `--cache-slots`, daemon `SLOT` line-prefix parsing, independent extra `TargetCache` state, slot graph/feature-mirror swapping, and cleanup/park handling. The remaining 6 non-draft PRs are still old conflict/selective-port candidates by exact-head ancestry (#305, #237, #221, #154, #153, and #135), but #305, #237, and #135 are now partially represented beyond their already-carried broad themes. Fresh direct-merge probes were rerun for all remaining non-ancestors on top of `2956d630`; conflict counts remained effectively unchanged except for the #135 probe still touching the same three files before the narrow slot scaffold was manually promoted.
...
| #324 | `codex/visible-empty-dflash-retry-upstream` | `34da50f` | included | Upstream/main-based visible-empty-DFlash-retry fix. Its effective tree changes were already carried by #319; merge conflict in `server/src/server/http_server.cpp` was resolved by preserving status-dashboard broadcasts plus #285 broader Gemma4 `<|channel>*` handling while recording exact PR-head ancestry. |
...
| #285 | `feat/lucebox-docker` | `d3399169` | included | Docker stack / `lucebox` CLI / harness / `luce-bench`, Bragi sweep docs, autotune/sweep updates, shell harness tests, long-context grader coverage, GPU-power-throttle notes, luce-bench grader fixes, think-vs-nothink baseline summary, Forge grader tests, workdir backup ignore rules, README wording refresh, Qwen closed-think/unit-comment updates, and the pFlash multi-turn session benchmark helper are carried exactly with current-stack conflict resolutions. The latest heads also broaden Gemma4 thinking-channel handling to raw tokens that start with `<|channel>`, add the server call-verb tool parser plan/implementation, and adjust Docker/unit-test guards, add the 2026-05-31 Bragi Qwen3.6 pFlash A/B experiment report, and update lucebox config path handling. |
...
The next useful integration work remains a dedicated selective port, not another direct merge. Highest-value candidates are now: (1) validate the newly promoted #135 daemon cache-slot scaffold under a populated CUDA/server build and add `LIST_TARGET_CACHE_SLOTS`; (2) continue #237 using the retained `/tmp/luce-port-pr237-qwen-mtp-20260531-083154` draft only after populating `server/deps/llama.cpp` or otherwise obtaining CI-backed compile evidence for the Qwen35 MTP module/graph/loader slice; (3) #135 runtime validation for the already-ported batched target-graph, request-tagged stream framing, and target-feature capture slices, then multi-cache-slot scaffolding, scheduler state/introspection, and diagnostic-only batch probing before live copyback/target-step mutation; (4) validate #305's newly ported Qwen35MoE gallocr-reuse/full-chunk FFN path under a populated CUDA server build before mining any remaining GPU-resident `act_cur` / async logits projection changes; and (5) #305's larger Laguna hot/cold MoE hybrid path if Laguna XS.2 residency is still desired. #153/#154 should be mined after #237 has current-layout Qwen35 runtime wiring for linear MTP decode semantics, and #221 only for any still-missing prefix-warm ideas. #137, #48, and #94 are represented by superseding current-layout code and can be closed or retargeted only if authors can identify a minimal missing delta.

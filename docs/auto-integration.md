...
|---:|---|---:|---|---|
| #305 | `layersplit_refactor` | `1de45e4d` | blocked-needs-human / human-scale refactor | Fresh direct merge still conflicts across 29 files: README/spec/harness docs, server CMake/scripts, backend IPC, backend precision, layer-split backend/adapters, Gemma4/Laguna/Qwen35/Qwen35MoE internals, and server tests. This run's tmux/Codex no-edit audit again found #305 unsafe for unattended integration: the final leaf commit (`perf(hybrid-prefill): remove sub-batching and reuse gallocr across layers`) is valuable but still touches already-conflicted Qwen35MoE backend code and depends on common MoE/Laguna state. Manual port should either scope narrowly to the Qwen35MoE gallocr/sub-batch optimization or intentionally resolve the full prerequisite stack; preserve current `supports_cpu_sampling()` / `sampler.needs_logit_processing()` layer-split gate, Qwen35 full-context `fa_window = 0` behavior, activation precision policy, shared IPC payload transport, `layer_split_runtime.cpp` CMake wiring, rich props endpoint metadata, current docs/specs/harness/CMake/IPC/precision/layer-split/Gemma/Qwen adapters, and deleted stale bench scripts unless explicitly revived. Run server build plus targeted `test_server_unit`, `test_dflash`, `test_generate`, and Qwen35MoE routing/placement/swap checks. |

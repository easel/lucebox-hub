...
Last refresh: `2026-05-31T12:51:12-04:00`
Current base: `origin/main` `8305b6c2`
Previous integration tip: `easel/auto-integration` `04199d8c`
Current integration source tip before this refresh: `04199d8c`
...
The current stack contains 23 exact current open non-draft PR heads and carries selective salvage from three remaining non-ancestor PRs: #305's `DFLASH_EXPERT_BUDGET_PCT` cap plus the current-layout Qwen35MoE batched-FFN gallocr reuse / caller-side `kFfnSafeBatch` removal, #237's common MTP interface/chain-runner/orchestrator foundation, and #135's capture-free `n_seqs` target-graph/cache plumbing, request-tagged daemon stream framing, batched target-feature capture buffer plumbing, daemon multi-target-cache-slot scaffolding, target-cache-slot/request introspection, cancellation scaffolding, and diagnostic-only `SCHED_STEP` / `SCHED_DRAIN` snapshots for current qwen35 work. There are currently 28 open non-draft PRs and 8 draft/excluded PRs. #285 advanced to `aa00f495` during this run and was merged with the existing status-dashboard / visible-output conflict resolution preserved.

...
Last refresh: `2026-05-31T13:39:39-04:00`
Current base: `origin/main` `8305b6c2`
Previous integration tip: `easel/auto-integration` `974d5cf9`
Current integration source tip before this refresh: `974d5cf9`
...
The current stack contains 26 exact current open non-draft PR heads and carries selective salvage from three remaining non-ancestor PRs: #305's `DFLASH_EXPERT_BUDGET_PCT` cap plus the current-layout Qwen35MoE batched-FFN gallocr reuse / caller-side `kFfnSafeBatch` removal, #237's common MTP interface/chain-runner/orchestrator foundation, and #135's capture-free `n_seqs` target-graph/cache plumbing, request-tagged daemon stream framing, batched target-feature capture buffer plumbing, daemon multi-target-cache-slot scaffolding, target-cache-slot/request introspection, cancellation scaffolding, and diagnostic-only `SCHED_STEP` / `SCHED_DRAIN` snapshots for current qwen35 work. There are currently 32 open non-draft PRs and 5 draft/excluded PRs. This refresh merged #291 exactly, which also represents its ancestor #290, adding draft residency policy and Gemma4 draft lifecycle support while preserving the current stack's PFlash passthrough, transitive-cascade, status/cancellation, and server-unit-test additions. #321 and #325 are now non-draft; they are not direct-merged yet because #325 supersedes #321 and conflicts heavily with current layer-split/IPC/runtime code, so they are classified as the next high-priority current-layout port target.
...
| #324 | `codex/visible-empty-dflash-retry-upstream` | `f540b29` | included | Current head is now carried exactly. It adds cancellation-on-disconnect plumbing through `CancelCallback`, `DaemonIO::should_cancel`, tokenizer cancellation, backend loops, SSE/header handling, and regression coverage while preserving auto-integration status-dashboard broadcasts, visible-output retry gating, layer-split runtime behavior, and #285 Gemma4 `<|channel>*` handling. |
...
| #291 | `feat-gemma4-draft-residency-followup` | `25f0efa1` | included | Superset of #290. Adds draft residency policy (`auto` / `persistent` / `request-scoped`), `--draft-residency` CLI and `/props.runtime` surfacing, PFlash and decode-draft request-scoped release actions, and Gemma4 draft-only park/unpark helpers while preserving current stack passthrough PFlash, transitive compression, cancellation, and status behavior. |
| #290 | `feat-server-draft-residency-policy` | `8cef6bb9` | included via #291 | Ancestor of #291; exact head is represented by the #291 merge. |
...
|---:|---|---:|---|---|
| #325 | `feat-layer-split-disk-prefix-cache` | `a043547f` | pending current-layout port / supersedes #321 | Newly non-draft. #325 is a strict superset of #321 (`origin/pr/321` is the merge-base) plus one disk-prefix-cache commit. Direct merge conflicts across the current stack's backend IPC, DFlash draft IPC, layer-split runtime, Gemma4/Laguna/Qwen35 layer-split adapters, backend IPC main, Laguna target loader, Qwen35 layer-split code, and disk-prefix-cache snapshot hooks. Next useful action: port #321's mixed-backend target-shard IPC/runtime support into current IPC/runtime APIs first, then port #325's `snapshot_ref` / `snapshot_adopt` same-backend target layer-split disk-prefix-cache support while keeping the mixed-backend `--kv-cache-dir` rejection. |
| #321 | `feat-mixed-backend-layer-split-runtime` | `71b3e983` | superseded by #325 / dependency payload | Newly non-draft. Do not merge separately unless splitting local commits for review; all #321 behavior is contained in #325. Unique missing behavior includes `RemoteTargetShardConfig`, `--target-shard-ipc-*`, `DevicePlacement::is_mixed_layer_split`, and Qwen35 target-shard IPC daemon/client plumbing. |
...
- `/tmp/luce-auto-cron-20260531-133052` (current source/manifest refresh worktree; #291/#290 promoted here, #321/#325 probed/classified)
...
The next useful integration work remains a dedicated selective port, not another direct merge. Highest-value candidates are now: (1) validate the newly promoted #135 daemon cache-slot, request-state, and scheduler-diagnostic scaffolds (`LIST_TARGET_CACHE_SLOTS`, `LIST_REQUESTS`, `CANCEL`, `SCHED_STEP`, and `SCHED_DRAIN`) under a populated CUDA/server build; (2) continue #237 using the retained `/tmp/luce-port-pr237-qwen-mtp-20260531-083154` draft only after populating `server/deps/llama.cpp` or otherwise obtaining CI-backed compile evidence for the Qwen35 MTP module/graph/loader slice; (3) #135 runtime validation for the already-ported batched target-graph, request-tagged stream framing, target-feature capture, slot/request control-plane, and scheduler diagnostics, then diagnostic-only batch probing before live copyback/target-step mutation; (4) validate #305's newly ported Qwen35MoE gallocr-reuse/full-chunk FFN path under a populated CUDA server build before mining any remaining GPU-resident `act_cur` / async logits projection changes; and (5) #305's larger Laguna hot/cold MoE hybrid path if Laguna XS.2 residency is still desired. #153/#154 should be mined after #237 has current-layout Qwen35 runtime wiring for linear MTP decode semantics, and #221 only for any still-missing prefix-warm ideas. #137, #48, and #94 are represented by superseding current-layout code and can be closed or retargeted only if authors can identify a minimal missing delta.

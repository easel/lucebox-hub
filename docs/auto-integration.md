...
Last refresh: `2026-05-31T15:22:07-04:00`
Current base: `origin/main` `8305b6c2`
Previous integration tip: `easel/auto-integration` `afeaf7ed`
Current integration source tip before this refresh: `afeaf7ed`
...
| #321 | `feat-mixed-backend-layer-split-runtime` | `87fe7655` | pending current-layout port / related to #325 | #321 has unique IPC/state hardening commit `87fe7655`; #325 is feature-superset-like but not an exact head superset. Unique missing behavior includes `RemoteTargetShardConfig`, `--target-shard-ipc-*`, `DevicePlacement::is_mixed_layer_split`, Qwen35 target-shard IPC daemon/client plumbing, state reset/snapshot/save/restore hardening, and the latest target-shard shared-payload / prefix-snapshot cleanup deltas. This run reprobed the direct merge (31 status entries / 12 unmerged paths). The next useful action remains a staged selective port of the target-shard IPC control plane before reattempting remaining #325 mixed-backend/Laguna disk-cache pieces. |

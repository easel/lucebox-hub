...
Last refresh: 2026-05-29T23:26:55-04:00
Current base: `origin/main` `8782d07a`
Current integration tip before this refresh: `easel/auto-integration` `4554979e`
Manifest refresh and unresolved-PR probe update prepared in this run: this commit

This branch is maintained as a reproducible patch stack over `origin/main`. This unattended run started from a clean primary checkout on `auto-integration`, verified GitHub/Claude/Codex auth with the real user credential home, fetched `origin` and `easel` separately, confirmed `origin/main` was already included, and performed fresh direct-merge probing in `/tmp/luce-auto-cron-20260529-232655/probes/pr-*-probe` worktrees.

No new non-draft contributor PR head required a code stack rewrite in this run. The current stack already contains the latest heads of #310, #309, #307, #306, #297, #295, #294, #289, #285, #276, #274, #266, #152, and #142. Fresh direct-merge probes for the remaining old-layout PRs still conflict with the same old `dflash/` to current `server/` restructuring and dependent MTP/scheduler areas. No new delegation was launched because the unresolved PR heads and conflict classes are unchanged from the immediately prior run; the latest usable delegated #237 report remains `/tmp/luce-auto-cron-20260529-220930/probes/codex-pr237-slice-report.txt`. Codex confirmed direct merge is unsafe and narrows the smallest buildable salvage slice to common MTP orchestration only, excluding qwen35 native-head runtime integration until a separate manual port preserves the current remote-draft, pFlash/C2, qwen35moe, budget, and GGUF-loader behavior.
...
- `date -Is` -> `2026-05-29T23:26:55-04:00` during preflight and manifest refresh.
...
- `origin/main` remains included in `auto-integration` (`git merge-base --is-ancestor origin/main HEAD`).
- Fresh probe worktrees attempted direct merges for every non-integrated non-draft PR. All eight still conflict in old-layout or dependent MTP/scheduler areas; logs are retained under `/tmp/luce-auto-cron-20260529-232655/probes/pr-*-merge.log`, status snapshots under `/tmp/luce-auto-cron-20260529-232655/probes/pr-*-status.txt`, and conflict-file lists under `/tmp/luce-auto-cron-20260529-232655/probes/pr-*-conflicts.txt`.
...
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-237-probe`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-221-probe`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-154-probe`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-153-probe`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-137-probe`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-135-probe`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-94-probe`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-48-probe`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-*-merge.log`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-*-status.txt`
- `/tmp/luce-auto-cron-20260529-232655/probes/pr-*-conflicts.txt`

Useful prior probe/delegation artifacts remain for unresolved old-layout PRs:

- `/tmp/luce-auto-cron-20260529-220930/probes/claude-pr237-slice-report.txt` (empty; Claude blank/stopped)
- `/tmp/luce-auto-cron-20260529-220930/probes/claude-pr237-slice-status.txt` (Claude failure note)
- `/tmp/luce-auto-cron-20260529-220930/probes/codex-pr237-slice-report.txt` (usable read-only #237 current-layout slice report)
- `/tmp/luce-auto-cron-20260529-214904/probes/pr-*-merge.log` and `pr-*-status.txt` (previous direct probes)

...
Neither of these requires secret sauce to understand. They're the obvious moves once one company controls all three layers, and the public descriptions of how these systems are built line up with both. Nothing here is clever or hidden. Vertical integration simply makes a class of optimization available, and the closed vendors took it.

## Where lucebox sits

This is where I should be plain about the open-weights position, because it's a constraint we can't engineer around and there's no use pretending the gap isn't there.

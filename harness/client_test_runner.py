...
def start_server(
    ...
    *,
    ...
    if backend != "cpp":
        raise RuntimeError(
            f"Unsupported LUCEBOX_SERVER_BACKEND={backend!r}; this worktree uses the native server/ layout."
        )

    # cpp backend: use the native dflash_server binary from server/build by default.
    cpp_bin = bin_path
    if not cpp_bin.exists():
        cpp_bin_env = os.environ.get("DFLASH_SERVER_BIN", "")
        cpp_bin = Path(cpp_bin_env) if cpp_bin_env else (ROOT / "server" / "build" / "dflash_server")
    if not cpp_bin.exists():
        raise RuntimeError(
            f"C++ server binary not found: {cpp_bin}\n"
            "Build it with `cmake --build server/build --target dflash_server` "
            "or set DFLASH_SERVER_BIN."
        )
    args = [
        str(cpp_bin),
        str(target),
...
class CodexAdapter(_BaseAdapter):
    ...
    def preflight_env(self) -> dict[str, str]:
        """Use an isolated HOME for preflight so live-run config does not pollute user state."""
        env = os.environ.copy()
        preflight_home = tempfile.mkdtemp(prefix=f"lucebox-{self.client}-preflight-")
        env["HOME"] = preflight_home
        if self.client == "codex":
            env["CODEX_HOME"] = preflight_home
        return env
